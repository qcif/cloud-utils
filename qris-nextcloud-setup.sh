...
# QRIScloud Nextcloud details
WEBDAV_URL=https://nextcloud.qriscloud.org.au/remote.php/webdav/
...
Options:
  -c     unmount and clean up any QRIScloud Nextcloud config
...
if [ "$cleanup" = true ]; then
  echo "Cleaning up QRIScloud Nextcloud setup..."
...
if mountpoint -q $MOUNT_PATH; then
  echo "QRIScloud Nextcloud already mounted at $MOUNT_PATH"
  exit 2
fi

  cat << EOF
This command guides you through the process of mounting your QRIScloud Nextcloud 
storage to your Nectar instance.

To mount your QRIScloud Nextcloud storage, you will be prompted for a username and
password. You can generate specific use 'App passwords' from the security
settings of your QRIScloud Nextcloud account here:
  https://nextcloud.qriscloud.org.au/index.php/settings/personal#security

PLEASE NOTE: If you snapshot this instance and share it, your QRIScloud Nextcloud 
...
echo -n "Testing your QRIScloud Nextcloud credentials... "
...
case $statuscode in
  ...
  401|503)
    echo "Error!"
    echo "Authentication error when testing your QRIScloud Nextcloud credentials."
    echo "Please check your credentials and try again later"
    exit 1
    ;;
  *)
    echo "Error!"
    echo "Unknown error when test your QRIScloud Nextcloud credentials."
    exit 1
    ;;
esac
...
# Explicitly disable locks - not supported by NextCloud
sudo sed -i 's/.*use_locks.*/use_locks 0/g' /etc/davfs2/davfs2.conf

# Perform mount
echo -n "Mounting QRIScloud Nextcloud storage... "
if sudo mount $MOUNT_PATH; then
  echo "done"
  echo "Your QRIScloud Nextcloud account has been mounted to $MOUNT_PATH"
  exit 0
fi

echo
echo "There was an error mounting your QRIScloud Nextcloud storage."
echo "For assistance, please copy the output of this command and submit"
echo "a ticket with our helpdesk at:"
echo "  support@qriscloud.org.au"
exit 1
